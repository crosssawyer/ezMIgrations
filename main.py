...
class SqlExtractor:
    ...
    def process_custom_sql(self, up_content: str, down_content: str) -> None:
        ...
        for sql in up_sql_matches + down_sql_matches:
            if self.is_stored_procedure(sql):
                self._handle_stored_procedure(sql)
    
    def _handle_stored_procedure(self, sql_content: str) -> None:
        proc_name = self._extract_procedure_name(sql_content)
        if proc_name:
            if proc_name in self.stored_procedures:
                self.stored_procedures[proc_name].update_up_method(sql_content)
            else:
                self.stored_procedures[proc_name] = StoredProcedure(
                    proc_name, sql_content, ""
                )
...
class MigrationManager:
    ...
    def generate_squashed_migration(self) -> str:
        ...
        migration_content += "protected override void Up(MigrationBuilder migrationBuilder)\n{\n"
        
        for proc in self.sql_extractor.stored_procedures.values():
            migration_content += f"    // {proc.name}\n"
            migration_content += f"    migrationBuilder.Sql(@\"{proc.latest_up_method}\");\n\n"
        
        migration_content += "}\n\n"
        migration_content += "protected override void Down(MigrationBuilder migrationBuilder)\n{\n"
        
        for proc in self.sql_extractor.stored_procedures.values():
            migration_content += f"    // {proc.name}\n"
            migration_content += f"    migrationBuilder.Sql(@\"{proc.oldest_down_method}\");\n\n"
